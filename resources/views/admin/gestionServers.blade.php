...
<body class="flex">
  ...
  <main class="bg-gray-100 p-4 flex-1">
    ...
    <section class="bg-white rounded-lg shadow-md p-4 flex">
      <!-- Table -->
      <table class="w-full table-auto m-5">
        <thead>
          <tr>
            <th class="px-4 py-2 text-left w-1/3">Serveur</th>
            <th class="px-4 py-2 text-left w-1/3">Nom de serveur</th>
            <th class="px-4 py-2 text-left w-1/3">AdressIp</th>
            <th class="px-4 py-2 text-left w-1/3">Module</th>
            <th class="px-4 py-2 text-left w-1/3">Gestion de serveur</th>
          </tr>
        </thead>
        <tbody>
            @foreach($serveurs as $serveur)
            <tr>
              <td class="px-4 py-2 border-b border-gray-200 w-1/3">{$serveur->idServeur}</td>
              <td class="px-4 py-2 border-b border-gray-200 w-1/3">{$serveur->dbName}</td>
              <td class="px-4 py-2 border-b border-gray-200 w-1/3">{$serveur->adresseIP}</td>
              <td class="px-4 py-2 border-b border-gray-200 w-1/3">{$serveur->type }</td>
              <td class="px-4 py-2 border-b border-gray-200 w-1/3">
                <a  href="{{ route('serveurs.edit', $serveur->idServeur) }}" class="bg-blue-700 text-white px-4 py-2 rounded-md hover:bg-blue-900 focus:outline-none focus:ring-2 focus:ring-blue-500">Modifier</a>
                <a href="{{ route('serveurs.destroy', $serveur->idServeur) }}" class="bg-red-700 text-white px-4 py-2 rounded-md hover:bg-red-900 focus:outline-none focus:ring-2 focus:ring-blue-500" onclick="event.preventDefault(); document.getElementById('delete-form-{{ $deploiment->id }}').submit();">Supprimer</a>
                <form id="delete-form-{{ $serveur->idServeur }}" action="{{ route('serveurs.destroy', $serveur->idServeur) }}" method="POST" style="display: none;">
                    @csrf
                    @method('DELETE')
                </form>            
              </td>
